import { Game, League, Player, Sport } from '../domain/types';
...
function player(id: string, name: string, team: string, position: string): Player {
  return { id, name, team, position };
}

interface GameInput extends Omit<Game, 'players'> {
  players: Player[];
}
...
export function getSampleSports(): Sport[] {
  return sports;
}
